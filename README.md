...
This repo includes a reference relay implementation under [`relay-server/`](relay-server/README.md). **The relay is not automatic** — you must add the route to your webhook server (or run the reference server) and allow CORS from your CloudTAK origin (e.g. `https://map.example.com`).
...
4. Deploy `GET /events/skydio/telemetry/:flightId` on the webhook server and set `CORS_ORIGINS` to include `https://map.example.com` (or your CloudTAK UI origin).
